function [curr_dat_sz, curr_lab_sz] = store2hdf5(filename, data, labels, create, startloc, chunksz)  
  ...
  if create
    ...
    h5create(filename, '/label', [Inf], 'Datatype', 'single', 'ChunkSize', [chunksz]); % width, height, channels, number 
  ...
  if ~isempty(data)
    ...
    h5write(filename, '/label', single(labels), startloc.lab, size(labels,2));  
  end

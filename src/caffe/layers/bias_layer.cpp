...
//function: make bias in rot_mode, when bottom[0]=top[0];
//todo: 
...
void BiasLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  /************************************************************/
	  ...
	  Dtype* bias_data_mut =this->blobs_[0].get()->mutable_cpu_data();
	  Dtype average_temp;
	  for( int i = 0; i < bias_dim_; i+=4){
		average_temp=(bias_data_mut[i]+bias_data_mut[i+1]+bias_data_mut[i+2]+bias_data_mut[i+3])/4;
	
		bias_data_mut[i]=average_temp;
		bias_data_mut[i+1]=average_temp;
		bias_data_mut[i+2]=average_temp;
		bias_data_mut[i+3]=average_temp;
	  }
  }
/************************************************************/	

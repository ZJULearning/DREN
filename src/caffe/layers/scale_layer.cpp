...
/************************************************************/
  if(rot_mode_ && (bottom.size() == 1)){
	Dtype* scale_data_mut =this->blobs_[0].get()->mutable_cpu_data();
	Dtype factor_average;
	for (int d = 0; d < scale_dim_; d+=4){
		factor_average = (scale_data_mut[d] + scale_data_mut[d+1] + scale_data_mut[d+2] + scale_data_mut[d+3]) / 4;
		scale_data_mut[d]=factor_average;
		scale_data_mut[d+1]=factor_average;
		scale_data_mut[d+2]=factor_average;
		scale_data_mut[d+3]=factor_average;
	}
	if (bias_layer_) {
		Dtype* scale_data_mut =this->blobs_[1].get()->mutable_cpu_data();
		Dtype factor_average;
		for (int d = 0; d < scale_dim_; d+=4){
			factor_average = (scale_data_mut[d] + scale_data_mut[d+1] + scale_data_mut[d+2] + scale_data_mut[d+3]) / 4;
			scale_data_mut[d]=factor_average;
